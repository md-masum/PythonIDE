...
var loadPyodide = ( () => {
    ...
    ;
    var b = (e, t, i) => (i = e != null ? ne(oe(e)) : {},
    $(t || !e || !e.__esModule ? F(i, "default", {
        value: e,
        enumerable: !0
    }) : i, e))
      , ce = e => $(F({}, "__esModule", {
...
`).filter(function(n) {
                ...
                return a.map(function(n) {
                    ...
                    var o = n.replace(/^\s+/, "").replace(/\(eval code/g, "(").replace(/^.*?\s+/, "")
                      , c = o.match(/ (\(.+\)$)/);
                    o = c ? o.replace(c[0], "") : o;
...
`).filter(function(n) {
    ...
    var K, U, Y, J, C;
    async function M() {
        if (!y || (K = (await import(/* webpackIgnore */
        "node:url")).default,
        J = await import(/* webpackIgnore */
        "node:fs"),
        C = await import(/* webpackIgnore */
        "node:fs/promises"),
        Y = (await import(/* webpackIgnore */
        "node:vm")).default,
        U = await import(/* webpackIgnore */
        "node:path"),
        W = U.sep,
        typeof p < "u"))
            return;
        let e = J
          , t = await import(/* webpackIgnore */
        "node:crypto")
          , i = await import(/* webpackIgnore */
        "ws")
          , r = await import(/* webpackIgnore */
        "node:child_process")
          , a = {
            fs: e,
            crypto: t,
    ...
    y || (W = "/");
    function be(e, t) {
        return e.startsWith("file://") && (e = e.slice(7)),
        e.includes("://") ? {
            response: fetch(e)
        } : {
            binary: C.readFile(e).then(i => new Uint8Array(i.buffer,i.byteOffset,i.byteLength))
        }
    }
    ...
    var R;
    if (V)
        R = s(async e => await import(/* webpackIgnore */
        e), "loadScript");
    else if (q)
        R = s(async e => {
            try {
    ...
    async function he(e) {
        ...
        e.includes("://") ? Y.runInThisContext(await (await fetch(e)).text()) : await import(/* webpackIgnore */
        K.pathToFileURL(e).href)
    }
    s(he, "nodeLoadScript");
    async function X(e) {
        if (y) {
            await M();
            let t = await C.readFile(e, {
                encoding: "utf8"
            });
            return JSON.parse(t)
        } else
    ...
    async function Q() {
        ...
        if (y && !t.startsWith("file://") && (t = `file://${t}`),
        H) {
            let r = await import(/* webpackIgnore */
            "node:path");
            return (await import(/* webpackIgnore */
            "node:url")).fileURLToPath(r.dirname(t))
        }
        ...
        let t = e.FS
          ...
          , a = {
            ...
            loadLocalEntry: n => {
                ...
                if (t.isFile(l.mode))
                    return c.contents = i.getFileDataAsTypedArray(c),
                    {
                        timestamp: l.mtime,
                        mode: l.mode,
                        contents: c.contents
            ...
            storeLocalEntry: (n, o) => {
                if (t.isDir(o.mode))
                    ...
                    t.writeFile(n, o.contents, {
                        canOwn: !0
                    });
                else
                    throw new Error("node type not supported");
                t.chmod(n, o.mode),
                t.utime(n, o.timestamp, o.timestamp)
            }
            ,
            removeLocalEntry: n => {
                var o = t.stat(n);
            ...
            storeRemoteEntry: async (n, o, c) => {
                let l = n.get(r.dirname(o))
                  , d = t.isFile(c.mode) ? await l.getFileHandle(r.basename(o), {
                    create: !0
                }) : await l.getDirectoryHandle(r.basename(o), {
                    create: !0
                });
                if (d.kind === "file") {
                    let u = await d.createWritable();
                    await u.write(c.contents),
                    await u.close()
                }
                n.set(o, d)
            }
            ...
            removeRemoteEntry: async (n, o) => {
                await n.get(r.dirname(o)).removeEntry(r.basename(o)),
                n.delete(o)
            }
            ,
            reconcile: async (n, o, c) => {
                ...
                Object.keys(o.entries).forEach(function(f) {
                    let v = o.entries[f]
                      , N = c.entries[f];
                    (!N || t.isFile(v.mode) && v.timestamp.getTime() > N.timestamp.getTime()) && (d.push(f),
                    l++)
                }),
                d.sort();
                let u = [];
                if (Object.keys(c.entries).forEach(function(f) {
                    o.entries[f] || (u.push(f),
                    l++)
                }),
                u.sort().reverse(),
                !l)
    ...
    async function B(e={}) {
        ...
        let a = ee(r)
          , n = a.API;
        if (n.lockFilePromise = X(r.lockFileURL),
        ...
        if (e.pyproxyToStringRepr && n.setPyProxyToStringMethod(!0),
